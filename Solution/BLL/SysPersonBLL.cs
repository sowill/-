    ...
    public class SysPersonBLL : IBLL.ISysPersonBLL, IDisposable
    {
            ...
            if (total > 0)
            {
                ...
                else
                {
                    ...
                    { 
                        ...
                        if (item.FileUploader != null)
                        {
                            item.FileUploaderId = string.Empty;
                            foreach (var it in item.FileUploader)
                            {
                                item.FileUploaderId += it.Name + ' ';
                            }                         
                        } 
       ...
       public bool Create(ref ValidationErrors validationErrors, SysEntities db, SysPerson entity)
        {   
            ...
            {
                FileUploader sys = new FileUploader { Id = item };
      ...
      /// <summary>
       ...
       public bool Edit(ref ValidationErrors validationErrors, SysEntities db, SysPerson entity)
        {  /*                       
            ...
            List<string> addFileUploaderId = new List<string>();
            List<string> deleteFileUploaderId = new List<string>();
            DataOfDiffrent.GetDiffrent(entity.FileUploaderId.GetIdSort(), entity.FileUploaderIdOld.GetIdSort(), ref addFileUploaderId, ref deleteFileUploaderId);
            if (addFileUploaderId != null && addFileUploaderId.Count() > 0)
            {
                foreach (var item in addFileUploaderId)
                {
                    FileUploader sys = new FileUploader { Id = item };
                    db.FileUploader.Attach(sys);
                    editEntity.FileUploader.Add(sys);
                    count++;
                }
            }
            if (deleteFileUploaderId != null && deleteFileUploaderId.Count() > 0)
            {
                List<FileUploader> listEntity = new List<FileUploader>();
                foreach (var item in deleteFileUploaderId)
                {
                    FileUploader sys = new FileUploader { Id = item };
                    listEntity.Add(sys);
                    db.FileUploader.Attach(sys);
                }
                foreach (FileUploader item in listEntity)
                {
                    editEntity.FileUploader.Remove(item);//查询数据库
                    count++;
                }
            } 
        ...
        /// <param name="id">主键</param>
        /// <returns>外键实体集合</returns>
        public List<FileUploader> GetRefFileUploader(string id)
        { 
            return repository.GetRefFileUploader(db, id).ToList();
        }
        /// <summary>
        /// 获取在该表中出现的所有外键实体
        /// </summary>
        /// <param name="id">主键</param>
        /// <returns>外键实体集合</returns>
        public List<FileUploader> GetRefFileUploader()
        { 
            return repository.GetRefFileUploader(db).ToList();
        }
