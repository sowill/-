    ...
    $.addFlex = function (t, p) {
        ...
        if (p.usepager) {
            ...
            if (p.searchitems) {
                ...
                $('#gridSearchBox').append("<div class='sDiv2'>Search <input type='text' size='30' name='q' class='qsbox' /> <select name='qtype'>" + sopt + "</select><input type='button' class='gridClear' value='Clear' /></div>");

                $('input[name=q],select[name=qtype]').keydown(function (e) { if (e.keyCode == 13) g.doSearch() });
                $('input[value=Clear]').click(function () { $('input[name=q]').val(''); p.query = ''; g.doSearch(); });
                //$(g.bDiv).after(g.sDiv);				
