    ...
    $.extend($t.datetime, {
        ...
        parseMachineDate: function (options) {
            ...
            if (baseDate == undefined) {
                ...
                date = new $t.datetime(year != -1 ? year : baseDate.year(),
                                       month != -1 ? month : baseDate.month(),
                                       day != -1 ? day : baseDate.date(),
    ...
    var Filter = {
        create: function (expressions) {
            ...
            for (idx = 0, length = expressions.length; idx < length; idx++) {
                ...
                operatorX = Filter.operatorX(expr.operatorX);
                desc = operatorX(selector, caseSensitive(expr.value));
        ...
        operatorX: function (operatorX) {
            if (!operatorX)
                return Filter.eq;
            if ($.isFunction(operatorX))
                return operatorX;

            operatorX = operatorX.toLowerCase();
            operatorXStrings = Filter.operatorXStrings;
            for (var op in operatorXStrings) {
                if ($.inArray(operatorX, operatorXStrings[op]) > -1) {
                    operatorX = op;
                    break;
                }
            }

            return Filter[operatorX];
        },
        operatorXStrings: {
        ...
        group: function (descriptors, allData) {
            ...
            if (descriptors.length > 0) {
                ...
                result = result.groupBy(descriptor).select(function (group) {
                    var data = new Query(allData).filter([{ field: group.field, operatorX: "eq", value: group.value}]);
                    return {
                        field: group.field,
                        value: group.value,
