(function ($) {
    ...
    $t.filtering.implementation = {
        createFilterCommands: function (html, column) {
            ...
            html.cat('<select class="t-filter-operatorX">');
        ...
        createFilterMenu: function (column) {
            ...
            $.each(column.filters || [], function (i) {
                $filterMenu.find('.t-filter-operatorX:eq(' + i + ')')
                           .val(this.operatorX)
                           .end()
                           .find(':text:eq(' + i + '),select:not(.t-filter-operatorX):eq(' + i + ')')
        ...
        filterClick: function (e) {
            ...
            $element.closest(".t-filter-options").find('input[type=text]:visible,select:not(.t-filter-operatorX)').each($.proxy(function (index, input) {
                var $input = $(input);
                ...
                var operatorX = $input.prev('select').val() || $input.parent().prev('select').val() || $input.parent().parent().prev('select').val();
                if (value != this.localization.filterSelectValue)
                    column.filters.push({ operatorX: operatorX, value: value });
            }, this));
            ...
            $element.parent().find('input:checked').each($.proxy(function (index, input) {
                ...
                column.filters.push({ operatorX: 'eq', value: value });
            }, this));
